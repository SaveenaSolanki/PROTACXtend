...
class SynGlue:
    ...
    def design_status(self, job_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if resp.status_code in (404, 500):
                print(f"Warning: Design job {job_id} does not exist or an internal error occurred.")
                return None
            else:
                raise
        return resp.json()
    ...
    def download_design(self, job_id: str, out_path: str) -> Optional[str]:
        ...
        try:
            ...
            if resp.status_code in (404, 500):
                print(f"Warning: For Design job {job_id} results are still not available, do not exist, or an internal error occurred.")
                return None
            else:
                raise
    ...
    def screen_status(self, job_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if resp.status_code in (404, 500):
                print(f"Warning: Screen job {job_id} does not exist or an internal error occurred.")
                return None
            else:
                raise
        return resp.json()
    ...
    def download_screen(self, job_id: str, out_path: str) -> Optional[str]:
        ...
        try:
            ...
            if resp.status_code in (404, 500):
                print(f"Warning: For Screen job {job_id} results are still not available, do not exist, or an internal error occurred.")
                return None
            else:
                raise
        with open(out_path, "wb") as f:
