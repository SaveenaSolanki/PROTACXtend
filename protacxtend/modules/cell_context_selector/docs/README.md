...
`python -m protacxtend.modules.cell_context_selector.dataset` writes
...
## Run
```
python -m protacxtend.modules.cell_context_selector.examples.train_demo \
    [--quick]
```
trains the production artifact (`models/cell_context_model.joblib`) and prints
the benchmark + claims + a demo prediction. Tests:
`pytest protacxtend/modules/cell_context_selector/tests/`.
