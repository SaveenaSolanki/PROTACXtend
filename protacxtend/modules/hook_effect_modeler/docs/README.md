...
Location: `protacxtend/modules/hook_effect_modeler/` (the `protacxtend`
distribution's code package is `protacxtend`, so the requested
...
```bash
python -m protacxtend.modules.hook_effect_modeler.examples.quickstart
python -m pytest protacxtend/modules/hook_effect_modeler/tests/
```

Agent/LangGraph: `protacxtend.tools.hook_effect_modeler_tool.run_hook_effect_modeler(payload)` —
