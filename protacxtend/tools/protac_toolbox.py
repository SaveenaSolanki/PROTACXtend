...
from protacxtend.backend.config import (
    DATA_DIR,
    DEFAULT_E3_LIGASES,
...
def _protacdb_exact_index() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from protacxtend.tools.protacdb_client import _load_normalized_protacdb_all
...
class ProtacDesignToolbox:
    ...
    def resolve_target(self, target_name: str, uniprot_id: str | None = None) -> TargetRecord:
        ...
        if query:
            try:
                from protacxtend.backend.uniprot_client import (
        ...
        if not target_rows:
            try:
                from protacxtend.tools.online_ligand_miner import (
    ...
    def expand_stereoisomers_controlled(
        ...
        for candidate in candidates:
            if len(expanded) >= max_total:
                break
            try:
                from protacxtend.tools.stereochemistry_engine import enumerate_stereoisomers, get_stereochemistry_profile
    ...
    def predict_degradation(
        ...
        if any(p.tack_dc50_nM is None for p in predictions):
            try:
                from protacxtend.tools.tack_degradation import predict_tack_batch
                tack_entries = [
    ...
    def predict_admet(self, candidates: Sequence[CandidateRecord]) -> list[ADMETPrediction]:
        from protacxtend.tools.admet_predictors import (
    ...
    def protacdb_evidence_prior(self, candidate: CandidateRecord) -> dict[str, Any]:
        ...
        """
        try:
            from protacxtend.tools.protacdb_client import load_normalized_protacdb, search_protacdb_evidence
        except Exception as exc:  # noqa: BLE001
    ...
    def score_e3_context(
        ...
        """
        from protacxtend.tools.e3_context_engine import score_e3
    ...
    def update_active_learning_from_feedback(
        ...
        registry = {
            ...
            "candidate_schema": "protacxtend.backend.schemas.CandidateRecord",
            "feedback_schema": "protacxtend.backend.schemas.AssayFeedbackRecord",
    ...
    def evolve_with_generations(
        ...
        from protacxtend.backend.schemas import GenerationRecord
