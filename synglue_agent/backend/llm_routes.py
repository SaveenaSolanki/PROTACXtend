...
def _status() -> Dict[str, Any]:
    from synglue_agent.llm.gateway import gateway_status
    return gateway_status()


def _switch(req: SwitchRequest) -> Dict[str, Any]:
    from synglue_agent.llm.gateway import switch_provider
    return switch_provider(req.provider, req.model, req.base_url, req.api_key)


def _providers() -> Dict[str, Any]:
    from synglue_agent.llm.providers import list_available_providers
    return {"providers": list_available_providers()}


def _models(provider: str = "") -> Dict[str, Any]:
    from synglue_agent.llm.providers import get_provider, get_config
    cfg = get_config()
    if provider:
...
def _test(req: TestRequest) -> Dict[str, Any]:
    ...
    from synglue_agent.llm.providers import ProviderConfig, get_config, set_runtime_config
    from synglue_agent.llm.schemas import EvidenceDecision
...
def _reset() -> Dict[str, Any]:
    from synglue_agent.llm.providers import reset_runtime_config
