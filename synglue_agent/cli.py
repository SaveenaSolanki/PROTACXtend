...
from synglue_agent.agents.runtime import run_protacpilot, summarize_run
from synglue_agent.backend.main import run_workflow_from_request, summarize_state, write_outputs
...
CAPABILITIES: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "status": "available",
        "detail": "Working engines behind run_retrosynthesis — synglue_agent/tools/retrosynthesis_engines.py; "
...
def _interactive_command() -> int:
    ...
    if sys.stdin.isatty():
        try:
            from synglue_agent.tui.app import launch_tui
            launch_tui()
            return 0
...
def _ui_command(args: argparse.Namespace) -> int:
    cmd = [
        ...
        "streamlit",
        "run",
        str(PROJECT_ROOT / "synglue_agent" / "app" / "streamlit_app.py"),
        "--server.address",
        args.host,
        "--server.port",
        str(args.port),
...
def _api_command(args: argparse.Namespace) -> int:
    cmd = [
        ...
        "-m",
        "uvicorn",
        "synglue_agent.backend.api_routes:get_app",
        "--factory",
        "--host",
        args.host,
        "--port",
...
def _status_command(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "frontend": {
            ...
            "entrypoint": "synglue_agent/app/streamlit_app.py",
        },
        "api": {
            "command": "PROTACXtend api",
            "url": "http://localhost:8001/docs",
            "entrypoint": "synglue_agent/backend/api_routes.py",
        },
