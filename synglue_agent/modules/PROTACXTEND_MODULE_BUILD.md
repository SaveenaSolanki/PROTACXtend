...
Layout (the `protacxtend` distribution's code package is `synglue_agent`, so the
requested `protacxtend/modules/<name>/` layout maps to
`synglue_agent/modules/<name>/` with separated `configs/ docs/ tests/ examples/`):

```
synglue_agent/modules/<module_name>/
    config.py, core.py, schemas.py, __init__.py
    configs/<module>.json
    docs/{README,ARCHITECTURE,USAGE,VALIDATION,LIMITATIONS,REFERENCES}.md
    tests/test_<module>.py
    examples/...
agent tool (LangGraph): synglue_agent/tools/<module>_tool.py  (JSON in/out, graph-safe)
```
...
- **Files added**: `synglue_agent/modules/hook_effect_modeler/` (core, schemas,
