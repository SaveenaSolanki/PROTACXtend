...
def pubchem_name_to_smiles(name: str) -> tuple[Optional[str], list[str]]:
    try:
        from synglue_agent.tools.pubchem_lookup import search_compound_by_name
...
def load_local_drugbank_binders(target_record: TargetRecord, drugbank_path: Path | None = None) -> tuple[list[BinderRecord], list[str]]:
    ...
    if not path.exists():
        return [], ["DrugBank fallback skipped: add licensed export to synglue_agent/data/drugbank_local.csv."]
