...
def resolve_target_with_provenance(
    ...
    *,
    ...
    if allow_network:
        try:
            from synglue_agent.tools.uniprot_lookup import get_uniprot_record, search_uniprot
...
def find_chembl_binders(
    ...
    *,
    ...
    from synglue_agent.tools.chembl_lookup import get_target_activities, rank_warhead_candidates, search_targets
...
def load_bindingdb_binders(target_query: str, *, path: str | Path | None = None, top_k: int = 100) -> dict[str, Any]:
    from synglue_agent.tools.bindingdb_lookup import search_bindingdb_local
...
def pubchem_compound_lookup(query_value: str | int, *, query_type: str = "name", allow_network: bool = False) -> dict[str, Any]:
    ...
    from synglue_agent.tools.pubchem_lookup import get_cid_from_smiles, get_compound_by_cid, search_compound_by_name
...
def pubchem_similarity_wrapper(smiles: str, *, threshold: int = 90, top_k: int = 20, allow_network: bool = False) -> dict[str, Any]:
    ...
    from synglue_agent.tools.pubchem_lookup import pubchem_similarity_search
